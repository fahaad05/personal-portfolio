import { getViewsSafely } from "@/lib/redis";
...
export default async function ProjectsPage() {
  const published = allProjects.filter(
    (p: any) => p && p.slug && p.published !== false
  );
  ...
  const views = await getViewsSafely(slugs);
  ...
  const featured =
    published.find((p: any) => p.slug === "personal-portfolio") ?? sortedAll[0];
  const top2 =
    published.find((p: any) => p.slug === "inventory-manager") ?? sortedAll[1];
  const top3 =
    published.find((p: any) => p.slug === "expenses-and-net-tracker") ??
    sortedAll[2];
  ...
  return (
    <div className="relative min-h-[100svh] bg-black">
      <div className="absolute inset-0 bg-gradient-to-tl from-zinc-900/0 via-zinc-900 to-zinc-900/0">
        ...
        <div className="relative px-6 pt-20 mx-auto space-y-8 max-w-7xl lg:px-8 md:space-y-16 md:pt-24 lg:pt-32 pb-16">
          <div className="max-w-2xl mx-auto lg:mx-0">
            <h2 className="text-3xl font-bold tracking-tight text-zinc-100 sm:text-4xl">
              Projects
            </h2>
            <p className="mt-4 text-zinc-400">
              Some of the projects are from university and some are on my own
              time.
            </p>
          </div>
